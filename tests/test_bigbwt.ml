...
let seq_BWT =
  In_channel.with_file "./data/big_bwt.txt" ~f:(fun channel ->
      In_channel.input_all channel)
...
let test_naive_bwt _ =
  ...
  assert_equal
    (paper_T |> CharBWT.getSA |> CharBWT.bwt_from_SA paper_T)
    paper_BWT;
...
let test_sais_sa _ =
  assert_equal
    ([| 1; 2; 2; 1; 2; 2 |] |> SaisBWT.getSA_int |> List.of_array)
    [ 6; 3; 0; 5; 2; 4; 1 ];
  assert_equal
    ([| 2; 2; 2; 2 |] |> SaisBWT.getSA_int |> List.of_array)
    [ 4; 3; 2; 1; 0 ]

let test_gsacak_sa _ =
  assert_equal
    ([| 1; 2; 2; 1; 2; 2 |] |> GsacakBWT.getSA_int |> List.of_array)
    [ 6; 3; 0; 5; 2; 4; 1 ];
  assert_equal
    ([| 2; 2; 2; 2 |] |> GsacakBWT.getSA_int |> List.of_array)
    [ 4; 3; 2; 1; 0 ]
...
let sa_int_tests =
  "sa_int_tests"
  >::: [ "sais sa" >:: test_sais_sa; "gsacak sa" >:: test_gsacak_sa ]

let series = "Project Tests" >::: [ pfp_tests; bwt_tests; sa_int_tests ]
